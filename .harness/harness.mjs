...
function checkHtml(file){const rel=path.relative(root,file).replaceAll('\\','/');const s=fs.readFileSync(file,'utf8');if(!/<html[^>]+lang=["'][^"']+["']/i.test(s))push('ERROR',rel,'HTML sem atributo lang.');if(!/<meta[^>]+name=["']viewport["']/i.test(s))push('ERROR',rel,'Viewport ausente.');if(!/<meta[^>]+name=["']description["']/i.test(s))push('ERROR',rel,'Meta description ausente.');if(!/<link[^>]+rel=["']canonical["']/i.test(s))push('ERROR',rel,'Canonical ausente.');if(rel==='index.html'&&!s.includes(`name="google-site-verification" content="${GOOGLE_SITE_VERIFICATION}"`)&&!s.includes(`content="${GOOGLE_SITE_VERIFICATION}" name="google-site-verification"`))push('ERROR',rel,'Verificação do Google Search Console ausente ou alterada.');const h1=(s.match(/<h1\b/gi)||[]).length;if(h1!==1)push('ERROR',rel,`Esperado 1 H1; encontrado ${h1}.`);for(const m of s.matchAll(/(?:href|src)=["']([^"']+)["']/gi)){const u=m[1];if(!u||u.startsWith('#')||u.startsWith('http:')||u.startsWith('https:')||u.startsWith('mailto:')||u.startsWith('tel:')||u.startsWith('data:'))continue;const clean=u.split(/[?#]/)[0];if(clean.startsWith('/'))continue;const target=path.resolve(path.dirname(file),clean);if(!fs.existsSync(target))push('ERROR',rel,`Referência local quebrada: ${u}`)}}
